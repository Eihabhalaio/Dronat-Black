...
build_container() {
    ...
    if sudo docker build -f Dockerfile.blackarch -t dronat-blackarch:latest .; then
...
test_container() {
    ...
    if sudo docker run --rm dronat-blackarch:latest python3 -c "print('Container test successful!')"; then
    ...
    for tool in "${tools[@]}"; do
        if sudo docker run --rm dronat-blackarch:latest which $tool &> /dev/null; then
    ...
    for lib in "${python_libs[@]}"; do
        if sudo docker run --rm dronat-blackarch:latest python3 -c "import $lib; print('$lib imported successfully')" &> /dev/null; then
...
stop_compose() {
    ...
    if [ -f docker-compose-blackarch.yml ]; then
        sudo docker-compose -f docker-compose-blackarch.yml down
...
cleanup() {
    ...
    if sudo docker ps -a | grep -q dronat-blackarch; then
        print_status "Stopping and removing BlackArch containers..."
        sudo docker stop $(sudo docker ps -aq --filter name=dronat-blackarch) || true
        sudo docker rm $(sudo docker ps -aq --filter name=dronat-blackarch) || true
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        sudo docker rmi dronat-blackarch:latest || true
    fi
